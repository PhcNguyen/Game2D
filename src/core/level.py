...
class Level:
	def __init__(self, tmx_map, level_frames, audio_files, data, switch_stage):
		...
		self.data = data
		self.switch_stage = switch_stage # Hàm chuyển đổi giai đoạn

		# Dữ liệu cấp độ
		self.level_width = tmx_map.width * TILE_SIZE # Chiều rộng level
		self.level_bottom = tmx_map.height * TILE_SIZE # Chiều cao level
		tmx_level_properties = tmx_map.get_layer_by_name('Data')[0].properties # Thuộc tính của layer 'Data'
		self.level_unlock = tmx_level_properties['level_unlock'] # Mở khóa level
		if tmx_level_properties['bg']:
			bg_tile = level_frames['bg_tiles'][tmx_level_properties['bg']] # Lấy tile nền
		else:
			bg_tile = None
		...
		self.collision_sprites = pygame.sprite.Group()  # Nhóm va chạm
		self.semi_collision_sprites = pygame.sprite.Group()  # Nhóm va chạm bán phần
		self.damage_sprites = pygame.sprite.Group()  # Nhóm đối tượng gây sát thương
		self.tooth_sprites = pygame.sprite.Group()  # Nhóm đối tượng 'tooth'
		self.pearl_sprites = pygame.sprite.Group()  # Nhóm đối tượng 'pearl'
		self.item_sprites = pygame.sprite.Group()  # Nhóm đối tượng 'item'

		self.setup(tmx_map, level_frames, audio_files) # Thiết lập level

		# Khung hình
		self.pearl_surf = level_frames['pearl'] # Khung hình của 'pearl'
		self.particle_frames = level_frames['particle'] # Khung hình của hiệu ứng hạt

		# Âm thanh
		self.coin_sound = audio_files['coin'] # Âm thanh của đồng xu
		self.coin_sound.set_volume(0.4)
		self.damage_sound = audio_files['damage'] # Âm thanh gây sát thương
		self.damage_sound.set_volume(0.5)
		self.pearl_sound = audio_files['pearl'] # Âm thanh của 'pearl'
	...
	def setup(self, tmx_map, level_frames, audio_files):
		...
		# Thiết lập các đối tượng di chuyển
		for obj in tmx_map.get_layer_by_name('Moving Objects'):
			if obj.name == 'spike':
				...
				if obj.width > obj.height: # Đối tượng di chuyển ngang
					move_dir = 'x'
					start_pos = (obj.x, obj.y + obj.height / 2)
					end_pos = (obj.x + obj.width,obj.y + obj.height / 2)
				else:  # Đối tượng di chuyển dọc
					move_dir = 'y'
	...
	def run(self, dt):
		...
		self.pearl_collision()      # Kiểm tra va chạm tới 'pearl'
		self.hit_collision()        # Kiểm tra va chạm gây sát thương
		self.item_collision()       # Kiểm tra va chạm với item
		self.attack_collision()     # Kiểm tra khi tấn công
		self.check_constraint()     # KIểm tra các giới hạn
